...
*   **`src/config.py`**: Custom config loader/validator from YAML (`TrainingConfig`), including multi-dataset mixes (`DatasetMixEntry`, `parse_data_mix_config`), DPO parameters (`dpo_beta`, `dpo_label_smoothing`), checkpoint path routing (`pretrained_model_path`, `ref_model_id`), and distributed parameters like `dist_timeout_seconds`.
*   **`src/dataset.py`**: Custom tokenizer training (`ByteLevelBPETokenizer`) with Gemma 3 turn tokens (`<start_of_turn>`, `<end_of_turn>`) and Jinja chat template, post-training tokenizer loader (`load_tokenizer_for_post_training`), multi-dataset proportional loading and mixing (`load_dataset_mix`), flat token packing with sliding window, atomic cache generation with `.ready` sentinels, and `DataLoader` creation.
*   **`src/sft_dataset.py`**: Turn-based conversational dataset (`SFTDataset`), multi-schema dialogue normalization (ShareGPT, Luciole, Comparia, FrenchQA/FQuAD), batched fast tokenization (`batch_format_messages_with_prompt_mask`), chunk progress verbosity, dynamic 2D batch collation (`SFTDataCollator`), and prompt loss masking (`labels = -100` on user/system prompt tokens) for supervised fine-tuning.
*   **`src/dpo_dataset.py`**: Preference dataset (`DPODataset`), preference pair loading (`load_dpo_pairs`), batched fast preference formatting (`batch_format_dpo_pairs`), dynamic 2D batch collation with length-based attention masks (`DPODataCollator`), and token log-probability extractor (`get_batch_logps`) for Direct Preference Optimization.
*   **`src/model.py`**: Wraps the base `Gemma3` model initialized from a blank configuration, adding a PyTorch-native `nn.Linear` LM Head. Incorporates **Gaussian embedding noise** (regulated by `embedding_noise_std`) during training. Enforces strict AutoModel hidden dimension and layer shape validation (`compare_and_load_automodel`) and vocabulary alignment (`ensure_tokenizer_vocab_alignment`) with device and dtype preservation.
*   **`src/scheduler.py`**: Handles linear warmup + cosine annealing with warm restarts, and `FreezeManager` layer freezing schedules.
*   **`src/trainer.py`**: Pretraining trainer loop driving optimization, evaluation metrics (loss & perplexity), text generation, and TensorBoard logging. Retains the **top 5 best checkpoints** based on evaluation metrics, deleting the worst when a better one is saved upon evaluation. Implements process-rank checks (`self.is_main_process`) and validation loss all-reduce for DDP synchronization.
