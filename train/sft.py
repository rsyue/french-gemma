...
def main() -> None:
    ...
    def save_if_better(step_idx: int, current_loss: float) -> None:
        if config.max_checkpoints <= 0:
            return
        checkpoint_name = f"sft_checkpoint_step_{step_idx}_loss_{current_loss:.4f}"
        ...
        if should_save:
            ...
            logger.info(f"Saved new best SFT checkpoint directory (loss={current_loss:.4f}): {checkpoint_path}")
