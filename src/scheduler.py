...
def get_lr_multiplier(
    step: int, warmup_steps: int, eta_min_ratio: float = 0.01, T_0: int = 1000, T_mult: int = 2
) -> float:
    """
    ...
    # Calculate current restart period cycle progress
...
def get_cosine_warmup_scheduler(
    optimizer: torch.optim.Optimizer, warmup_steps: int, T_0: int = 1000, T_mult: int = 2, eta_min_ratio: float = 0.01
) -> LambdaLR:
    """
    ...
    def lr_lambda(step: int) -> float:
        return get_lr_multiplier(
            step=step, warmup_steps=warmup_steps, eta_min_ratio=eta_min_ratio, T_0=T_0, T_mult=T_mult
        )
